...
const updateCollection = async (req, res) => {
    ...
    const collection = await Collections.findOne({ _id: collectioId }, { owner: 1 });
    if (!collection) {
    ...
    if (!isTypeValid) {
        return res
            .status(400)
            .json({ message: 'Collection Type is not valid' });
    }
...
const createCollection = async (req, res) => {
    const { title, description, description_text, allowedFields,_id, updImg } = req.body;
